...
fn add_cell_paths(paths: &mut Vec<Path>, maze: &Maze, (row, col): Cell, walls: &HashSet<Wall>) {
  ...
  for &wall in walls {
    match wall {
      Top => {
        let path = make_line(left_corner, (CELL_SIDE, 0), "black");
        paths.push(path)
      }
      Left => {
        let path = make_line(left_corner, (0, CELL_SIDE), "black");
        paths.push(path)
      }
      // only draw right and bottom for right and bottom edges, to avoid double lines
      Right => {
        if col == maze.width() - 1 {
          let path = make_line((left_corner_x + CELL_SIDE, left_corner_y), (0, CELL_SIDE), "black");
          paths.push(path);
        }
      }
      Bottom => {
        if row == maze.height() - 1 {
          let path = make_line((left_corner_x, left_corner_y + CELL_SIDE), (CELL_SIDE, 0), "black");
          paths.push(path);
        }
      }
    };
